...
async fn get_test_user_id(client: &Client) -> i64 {
    ...
    let track = client
        .get_track(&identifier)
        .await
        .expect("Failed to get track");
    track.user.and_then(|u| u.id).expect("Track has no user ID")
}
...
async fn test_refresh_client_id() {
    ...
    client
        .refresh_client_id()
        .await
        .expect("Failed to refresh client ID");
...
async fn test_search_tracks() {
    ...
    assert!(
        !tracks.collection.is_empty(),
        "Should return at least one track"
    );
}
...
async fn test_get_track_related() {
    ...
    let result = client
        .get_track_related(&identifier, Some(&pagination))
        .await;
    assert!(
        result.is_ok(),
        "get_track_related with pagination should succeed"
    );
}
...
async fn test_get_stream_url() {
    ...
    assert!(
        ...
        "Stream URL should be a valid HTTP URL"
    );

    // Test with Progressive stream type
    let result = client
        .get_stream_url(&identifier, Some(&StreamType::Progressive))
        .await;
    assert!(
        result.is_ok(),
        "get_stream_url with Progressive should succeed"
    );
}
...
async fn test_get_track_waveform() {
    ...
    assert!(
        waveform.samples.is_some() || waveform.width.is_some(),
...
async fn test_get_user_followers() {
    ...
    let result = client
        .get_user_followers(&identifier, Some(&pagination))
        .await;
    assert!(
        result.is_ok(),
        "get_user_followers with pagination should succeed"
    );
}
...
async fn test_get_user_followings() {
    ...
    let result = client
        .get_user_followings(&identifier, Some(&pagination))
        .await;
    assert!(
        result.is_ok(),
        "get_user_followings with pagination should succeed"
    );
}
...
async fn test_get_user_playlists() {
    ...
    let result = client
        .get_user_playlists(&identifier, Some(&pagination))
        .await;
    assert!(
        result.is_ok(),
        "get_user_playlists with pagination should succeed"
    );
}
...
async fn test_get_user_tracks() {
    ...
    assert!(
        result.is_ok(),
        "get_user_tracks with pagination should succeed"
    );
}
...
async fn test_get_user_reposts() {
    ...
    let result = client
        .get_user_reposts(&identifier, Some(&pagination))
        .await;
    assert!(
        result.is_ok(),
        "get_user_reposts with pagination should succeed"
    );
}
...
async fn test_search_playlists() {
    ...
    assert!(
        !playlists.collection.is_empty(),
        "Should return at least one playlist"
    );
}
        ...
        .await
    {
        if let Some(playlist) = playlists.collection.first() {
            if let Some(playlist_id) = playlist.id {
                ...
                let result = client
                    .get_playlist_reposters(&identifier, Some(&pagination))
                    .await;
                assert!(
                    result.is_ok(),
                    "get_playlist_reposters with pagination should succeed"
                );
                return;
            }
        }
